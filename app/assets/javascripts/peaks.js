...
$(function(){
  ...
  $('#message').text("You are looking peaks of the " + type + " of " + date);
});
...
function setchart(date){
  ...
  var type = $("input:radio[name=options]:checked").val();
  $.getJSON("/reports/peaks/"+date +"/" +type, function(data) {
    console.log(data);
    var max = 0;
    var datos = [];
    for (var i = 0; i < data.length; i++){
      max = (max > data[i].max ? max : data[i].max); 
    }
    options.data = [];
    for (var i = 0; i < data.length; i++){
      data[i].part = Math.round((data[i].max/max*100)*100)/100; 
      options.data.push ({value : data[i].part, label : data[i].description });
      $("#"+data[i].ids).width((data[i].part)+"%");
      $("#"+data[i].ids).html((data[i].part)+"%");
      $("#"+data[i].ids).css('backgroundColor', options.colors[i]);
      $("#"+data[i].ids+"-value").html(numberWithDots(Math.round(data[i].max) + " Watts"));
    }
